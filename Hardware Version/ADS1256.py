...
class ADS1256:
    ...
    def ADS1256_ConfigADC(self, gain, drate):
        self.ADS1256_WaitDRDY()
        buf = [0,0,0,0,0,0,0,0]
        buf[0] = (0<<3) | (1<<2) | (1<<1)  # Set bit 1 high to enable Buffer
        #buf[0] = (0<<3) | (1<<2) | (0<<1)  # Set bit 1 low to disable Buffer
        buf[1] = 0x08
    ...
    def ADS1256_init(self):
        ...
        #self.ADS1256_ConfigADC(ADS1256_GAIN_E['ADS1256_GAIN_1'], ADS1256_DRATE_E['ADS1256_5SPS'])
        
        # Check if buffer was enabled
        status_reg = bin(self.ADS1256_Read_data(REG_E['REG_STATUS'])[0])
        print("Status Register: "+str(status_reg))
        return 0
